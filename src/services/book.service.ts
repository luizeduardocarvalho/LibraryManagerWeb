...
export class BookService {
  ...
  returnBook(book: GetBook): Observable<Transaction> {
    return this.http.post<Transaction>(baseUrl + 'books/return', book, this.httpOptions);
  }

  renewBook(getBook: GetBook): Observable<Transaction> {
    return this.http.patch<Transaction>(baseUrl + 'books/renew', getBook, this.httpOptions);
  }

  lendBook(lendBook: LendBook) {
    return this.http.post<LendBook>(baseUrl + 'books/lend', lendBook, this.httpOptions);
  }

  createBook(book: CreateBook) {
    return this.http.post<CreateBook>(baseUrl + 'books/create', book, this.httpOptions);
  }

  updateBook(book: UpdateBook) {
    return this.http.patch<CreateBook>(baseUrl + 'books/updatebook', book, this.httpOptions);
  }
}
